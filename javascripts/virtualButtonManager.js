const virtualButtonManager = (() => {
    ...
    const toggleEditMode = () => {
        ...
        overlayDiv.style.backgroundColor = 'rgba(0, 0, 0, 0.2)'; // 半透明黑色
    ...
    const handleBodyClick = (event) => {
        if (editMode) {
            // 检查是否点击在 completeButton 或 clearButton 上
            if (event.target === completeButton || event.target === clearButton) {
                return; // 如果是，直接返回，不生成临时按钮
            }
        ...
        if (key == "dpad") {
            ...
            button.addEventListener('mousedown', (event) => {
                if (editMode) {
                    ...
                    button.addEventListener('mousemove', handleMove); // 监听移动
                }
            });
            ...
            button.addEventListener('touchstart', (event) => {
                if (editMode) {
                    ...
                    button.addEventListener('touchmove', handleMove); // 监听移动
                }
            });
